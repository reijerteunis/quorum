...
already merged into"* `main` — **was accurate at the time, in both of its claims.** The ancestry
fact held, and so did the historical event: the branch had genuinely been merged into `main` by a
real merge commit, 24 hours and 50 minutes before the run began. What the erratum could not
reproduce on 2026-08-24 was an artefact of the branch having moved *after* that review, not of a
wrong diagnosis. M1's closing entry is therefore right on the substance and wrong only in its
interval — it says "hours earlier" where the record says a day and an hour.
...
| 2 | Where did `main` point during the run? | `git log main --format='%h %cI'` around that window | `cdec5e9` (`merge: Q-0033 review flow surface…`) at **00:47:33+02:00**, eleven minutes before the start; the next commit on `main`, `3790c04`, is **01:15:20+02:00** — after the end. So `main = cdec5e9` throughout. |
| 3 | Where did the ticket branch point during the run? | `git log -1 --format='%p' ebf1c6e` → `998f397 6cc9da4` | `ebf1c6e` (`Merge branch 'main' into harness/Q-0006/integration`, **01:38:48+02:00**) is the branch's next move after the run; its **first parent** is therefore the tip immediately before it: `998f397`, committed **2026-08-23T00:00:33+02:00**. The branch did not move between then and 01:38:48+02:00, so `harness/Q-0006/integration = 998f397` throughout. |
| 4 | Was that tip an ancestor of that base? | `git merge-base --is-ancestor 998f397 cdec5e9` | **exit 0** — contained. The ancestry fact was true. |
| 5 | By what route did it become reachable? | `git rev-list --ancestry-path 998f397..cdec5e9`, oldest entry | `5d16e06`, parents `a08fbfa 998f397`, `merge: Q-0006 review-flow runtime into main [Q-0006]`, **2026-08-23T00:07:52+02:00**. A two-parent commit whose second parent is `998f397` merged it. The historical event was true as well. |
| 6 | Why could the erratum not reproduce it? | same `git log` as #3 | `ebf1c6e` (**01:38:48+02:00**) and `29ad00a` (**01:42:05+02:00**) both postdate the run's end at 01:11:00+02:00. The branch acquired a merge of `main` and the runtime task's work *after* the review, which is exactly why `git diff --stat main...harness/Q-0006/integration` reported 45 insertions when the erratum looked. |
| 7 | And today? | `git branch --merged main --list 'harness/Q-0006/*'` | lists `harness/Q-0006/integration`, and the three-dot diff is empty again — Q-0034 landed it on 2026-08-24. |

Rows 2 and 3 carry the argument and neither uses the reflog, which is machine-local, expires by
default and does not survive a clone. Row 3 is the one to keep: **a merge commit's first parent is
where the branch was standing before it moved**, which recovers a ref's past position from committed
history alone.
...
Rows 4 and 5 are separate questions and both had to be asked. (b) Corroborating with `git reflog` —
...
2. **It named nothing a reader could check.** No SHA, no exit code, no statement of which check ran.
   Branch tips move — this one moved twice within half an hour of the run — and a message naming no
   SHA cannot be re-checked afterwards, which is the only time anyone wants to. Establishing rows 2
   and 3 above took an evening precisely because the message recorded none of it.
...
caught by a new static rule in `harness lint`. Q-0035 (OQ-1).

**Found by:** re-deriving the erratum, as Q-0034's closing entry assigned to Q-0035.
