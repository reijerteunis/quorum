...
*Status: definition locked after grilling session, 2026-08-06; positioning amended 2026-08-22 (launch via heyruud.com, product-agnostic, dogfood portfolio); renamed from Harness Studio to Quorum 2026-08-22. See DECISIONS.md for the reasoning behind every choice and GLOSSARY.md for terminology.*
...
Projects home (picker) · Backlog board (tickets by stage) · Harness editor (canonical sources, compile status per vendor, drift indicator) · Flow editor (form-based steps/gates) · Live run mission control (parallel trace columns, cost tickers) · Gate screen (verdict + side-by-side diffs + advance/re-run/override) · Step chat (grill interview) · Run history (list, trace drill-down, cost roll-ups).
