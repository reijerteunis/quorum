...
**Connection state**: The browser's closed, derived account of the live socket at each moment: `idle`, `connecting`, `live`, `no daemon`, `no such run`, `ended`, `interrupted`, `dropped`, or `protocol error`. It is held in memory and never persisted. **No member of it is silence.** Every one of the nine renders a sentence a reader can act on, which is why *no daemon* and *no such run* are separate members rather than one — they are "start the daemon" and "that handle is wrong" — and why an empty region is never one of the nine. It is not **run state**: connection state describes this browser's transport and can change without changing the daemon run.
...
five packages that emit. It is the **first task in this workspace whose `outputs` is non-empty**,
...
verdict moves behind it"* (2026-09-02), *"A fourth package emits, and what it emits is served
rather than shipped"* (2026-09-12), which took it from three packages to four, and *"A fifth package
