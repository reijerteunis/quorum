...
**Connection state**: The browser's closed, derived account of the live socket at each moment: `idle`, `connecting`, `live`, `no daemon`, `no such run`, `ended`, `interrupted`, `dropped`, or `protocol error`. It is held in memory and never persisted. It is not **run state**: connection state describes this browser's transport and can change without changing the daemon run.
