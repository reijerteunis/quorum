...
- Q-0018 Run history lists the runs that finished, and drills into one. *(`reviewed` and
  ...
  **AC-14's three document corrections landed with it**, including `GET /project` named as **still
  ...
  `quorum lint` 6/6.
- Q-0137 The drill-down serves what an occurrence retained. *(Opened 2026-09-18 at Q-0018's
  requirements gate, `draft`, p2, with Appendix A transcribed in full.)* The retained-file half:
  **1,779 files and 115,557,259 bytes across 171 runs**, and nothing in `packages/server` reads one.
  **Its confinement threat is not where a reader will look for it.** Q-0127's file route takes an
  untrusted `?path=` **from a client**; here the client supplies an occurrence *identity* and **the
  path comes out of the manifest**, which `readRun`'s own JSDoc calls *"a cast, never a check"* —
  `manifestShapeError` proves three strings and two arrays and **no occurrence field is validated
  anywhere on the read path**, with **no test in either package staging a traversing
  `occurrence_dir`**. A guard modelled on Q-0127's without reading that paragraph confines the wrong
  value. A new `core` function is **required rather than preferred**, `resolveRunDirectory` being
  deliberately off the barrel so a caller cannot resolve lexically and read anyway; and the file-name
  set is **open by construction**, `persist(occurrence, name, text)` taking the name as a parameter,
  so the directory is listed rather than two names hard-coded. **The payload is its first decision
  rather than an implementation detail**: `{name, bytes}` with one file on demand caps what a reader
  receives at **355,744 B**, where a run's text in one response is **3,514,617 B** and makes Q-0076
  this ticket's blocker instead of leaving it at p3.
