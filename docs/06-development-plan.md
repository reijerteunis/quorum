...
- Q-0074 A failed git probe is read as a proven negative. *(`requirements` 2026-09-10, ready at
  iteration 2 and answered `advance`; **split in three at that gate**.)* **$23.82** across two runs —
  the first died at `head-of-product` on `error_max_structured_output_retries`, the same vendor
  failure that killed Q-0039's round 4 a day earlier, and both candidates survived so the re-run cost
  only the candidates again.
  **The census is what the money bought.** Twenty-four `safe()` call sites — 16 in `git/git.ts`, 8 in
  `fanout/fanout.ts`, one primitive declared byte-for-byte twice — each classified by *what a failure
  becomes* rather than by whether it is wrapped: **thirteen collapse, eleven do not.** Four of the
  thirteen were named by no ticket, no comment and no test before this run: `git.ts:143`, where a
  failed base probe cuts the worktree from `HEAD` instead; **`git.ts:345`, where one failed
  `for-each-ref` empties the branch set so `quorum board` answers `no branch` for every ticket in the
  backlog** — the product's most-used command telling an adopter none of their work exists;
  `fanout.ts:280`, where a failed status probe reads `backlog/` as clean so no revert runs and
  `onDiscard` never fires; and `fanout.ts:317`, where a failed `merge --abort` leaves a merge in
  progress against a JSDoc promising *"leave the worktree clean either way"*. The engine's rollback
  has **two** routes rather than the one every earlier account recorded — `lifecycle.ts:136` and
  `:139`, both operands from `safe()`-wrapped reads.
  **The census also rotted in a day and says so.** The merge triage counted 23 on 2026-09-07; Q-0112
  added `git.ts:213` on 2026-09-08. So it ships as a register of identities in
  `backlog.source.test.ts`'s shape, asserting over classifications rather than over a total, and no
  criterion may depend on its count.
  **Decision entry 088 landed at the gate, before a line of code** — *"A probe that could not answer
  is not a negative"* (2026-09-10) — which is GO-1, the obligation whose omission cost real money
  three times (Q-0062, Q-0070, Q-0101). It rules the primitive once rather than per subject, and
  states the rule the census yields: **`safe()` is correct wherever the caller's question cannot tell
  its two inputs apart, and wrong wherever the caller acts differently on them; the primitive is not
  the defect, a caller that merges an unanswerable probe into a positive claim is.** It also settles
  **OQ-4** — a narrow filesystem check *may* separate an absent `.git` from an unreadable one, and
  *"Membership is a git question, not a filesystem one"* (2026-08-28) does not govern, on Q-0090
  E-1's precedent, with the bound stated.
  **The split, and why the id did not move.** *"What a run's event stream carries"* (2026-08-28)
  cites **Q-0074** by name against `branchExists`, `branchHead`, `commitAll` and `mergeInto`, and
  `composite.ts:17` says *"Q-0074 owns it"* — a landed entry is never edited, so this ticket keeps
  the `fanout/` half and its fourteen criteria. **Which half runs first is a separate question**, and
  the gate ruled `git/` on two measurements: it holds the only site that reaches an adopter's screen,
  and it is nearly mechanical because the vocabulary is already in the file.
  **The trap was set inside this ticket's own instrument:** the four pins recording its defects cite
  **`Q-0048`**, not `Q-0074`, and a search for `Q-0074` returns one line in the whole package.
