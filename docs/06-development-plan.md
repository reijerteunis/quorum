...
- Q-0039 One run at a time per ticket. *(`requirements` 2026-09-09, ready at the gate on iteration 2
  and answered `advance`; opened as a folder 2026-08-31, cited on this page since 2026-08-24.)*
  **$14.78** across two head-of-product iterations and one unpriced codex candidate. Thirteen
  criteria. **The ticket M1's and Q-0034's closing entries both say lands before M3's daemon makes
  concurrent runs ordinary** — and that is now the next milestone rather than a later one. Nothing
  serialises runs: re-verified 2026-09-09, there is no lock of any kind in `packages/core/src`, and
  two invocations against one ticket collide on the run id, the ticket branch and the worktree. It
  has happened, twice in one night, which is what opened it.
  **The deliverable is one file under `.quorum/`, created by one exclusive syscall, whose subject is
  the ticket**, taken inside `run()` before anything the run must keep and released in a `finally`
  covering all six exits. A second run refuses and names the holder rather than waiting. GO-1's
  entry landed before the chore run — *"A run holds a lock on its ticket, and a stale one refuses
  rather than being reclaimed"* (2026-09-09).
  **Its sharpest ruling is OQ-2, and one of its four grounds is a connection nobody had made:** a
  pid probe is **Q-0074's** open defect class, *probe fails → read as absence*, so shipping one here
  would add an instance while the ticket to remove them is unstarted. The other three are that
  `run.ts:179–180` aborts an `AbortController` rather than exiting, so SIGINT and SIGTERM both reach
  the `finally` and a leak needs SIGKILL (M-4); that containment, push lag and verified version each
  keep a state meaning *could not tell* while a boolean liveness answer has none; and that
  refuse-now is additive where reclaim-now cannot be un-shipped.
  **Iteration 1 returned `needs-input` on two blockers and iteration 2 cleared both on an unchanged
  tree** — the fourth recorded instance of *"a retry on an unchanged tree cannot rule its own
  blocker"* (Q-0090, Q-0096, Q-0105) and the **second** where the second pass found a way through
  anyway. Neither reversal is on the merits: OQ-1 was downgraded to a gate obligation on the role's
  own test — *an open question that would change the design*, which this is not, since the same file,
  syscall and refusal ship either way — and OQ-2 was ruled rather than re-asked. The codex
  candidate's AC-21, which stated the decision entry **as an acceptance criterion**, is struck under
  *"A requirement may not name a surface its flow cannot write"* (2026-08-25).
  **Q-0114 was opened at this gate**, not in a closing entry, which is the whole point of it.
