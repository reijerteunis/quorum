...
- Q-0018 Run history lists the runs that finished, and drills into one. *(`requirements` 2026-09-18,
  ready on the **first** head-of-product iteration at fourteen criteria; **cut in two at its gate**,
  with **Q-0137** taking the retained files.)* **$14.64** and 8,380,147 tokens across three steps
  plus one unpriced codex candidate. It keeps everything a screen can read from a **manifest**: a
  `WireRunHistoryList` for a route that has declared no shape since Q-0119, a widened listing, the
  table, and the brief's inline expansion into one run's occurrence timeline.
  **The seam is one step from where the ticket body put it, and the correction is the run's.** The
  body cut at *listing / drill-down*; measured, the brief's **inline expansion** — the occurrence
  timeline and the per-vendor split — needs no route, no file read, no confinement surface and no
  ruling, because `GET /history/:id` has existed since Q-0119 and `fetchRunHistory` already calls it.
  The disjoint seam is **manifests against retained files**, and it is checkable in code rather than
  only in prose: `occurrenceSeq` parses a directory *name* and opens nothing, so **this half never
  composes a filesystem path** and Q-0137 does.
  **Its central measurement refutes the design brief rather than deferring it.** §8 ends *"Clicking
  opens the trace (reuse screen 5 in a 'completed' state)"*, and screen 5 cannot be reused for a run
  on disk for two independent reasons. **Events are not persisted** — `writer.ts` names none and the
  glossary's **Event** term says so in as many words — while mission control's columns, lane and
  timeline are every one derived from accepted events off a socket. And the two identity schemes do
  not meet: a handle is *"deliberately meaningless across a restart"* and a history id is
  `<TICKET>-<n>`, so `.quorum/runs` holds **171** directories against **zero** handles in a freshly
  started daemon. The divergence is recorded in the brief (AC-14) on Q-0017's and Q-0129's precedent.
  **The run refuted its own ticket body in two places and the sharper one it created by measuring.**
  The body said `incomplete` is **0 of 170**; at the gate there are **171** and one is `running` with
  `ended_at: null` — **`Q-0018-1`, the run writing the document**. The case the body called
  untestable from this corpus is the state the corpus was in, so AC-4 and AC-11 are written against
  it rather than around it. (The body also said the brief asks for nine columns; it asks for eight.)
  **And it reframed its own blocking question by finding a sentence already in the tree.** The body
  framed *may a route serve a file under `.quorum/`?* as needing Q-0127's erratum E-1 extended.
  ...
  ruled that engine run state may not be served **from the backlog route** and forwarded this subject
  here **by name**. What is left is narrower and travels to Q-0137: does that forwarding survive the
  difference between an empty `.harness/` inside a ticket folder and 115 MB under `.quorum/runs` — **a
  payload question wearing an authority question's clothes**.
  **Four errata at the gate.** E-1 rules the split and names AC-4 and AC-14 as **not eligible for
  trimming** if the loop exhausts, in advance, which is Q-0122's discipline and what Q-0126 paid
  $177.92 for not having. E-2 opens Q-0137 with its id allocated rather than assumed. E-3 ratifies
  that **no decision entry is owed**, by applying the *does any landed sentence go false?* test at
  five sites — the only one that does is a source docblock whose own reasoning expires by its terms,
  `WireRunHistory`'s *"`steps` is deliberately absent"* resting on *"read by nothing that reads this
  shape"*. E-4 records the two body corrections and a third of its own: a figure in the merged
  document was **true when taken and false by the gate**, for the same reason the first one was.
