...
- Q-0118 The daemon speaks HTTP and WebSocket. *(Opened 2026-09-11 at Q-0013's requirements gate as
  its GO-5, `draft`, p2 — **successor A**, transcribed in full from that ticket's Appendix A.)* The
  three routes `04-architecture.md` already names, the WS envelope, request validation and status
  mapping, the loopback bind, the retention default, and the protocol contract Q-0014 codes against.
  Ordinary Hono work over a host whose behaviour Q-0013 has already proved — and **where the product
  decisions live**, which is why the full-pipeline argument Q-0013's gate deferred is routed here:
  its design is genuinely open where the host's was settled by four landed entries.
