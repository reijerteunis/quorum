...
- Q-0010 CLI package; `npx quorum` entry. *(`reviewed` 2026-09-06, closed by hand once all eleven
  ...
  **Each child body repeats Q-0010's ground rules verbatim**, because `input.backlog` resolves
  ...
  - Q-0111 May a `core` error message tell a user what to type. **Its decision entry landed
    2026-09-07** — *"A `core` error names the condition; the remedy belongs to the surface"* — so what
    remains is the code and the ticket stays open for it. **The entry disproved this ticket's own
