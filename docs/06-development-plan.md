...
- Q-0119 The server serves the project, the backlog, the flow set and run history. *(Opened
  2026-09-11 at the same gate, `draft`, p3 — **successor B**, Appendix B in full.)* The read-only
  REST surface: what project is open, what tickets exist, which flows are runnable, what previous
  runs did. **It needs nothing added to `core`** — every symbol is already on the barrel — which is
  why it goes last rather than first.
