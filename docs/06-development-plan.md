...
- Q-0116 A dry run may not mutate the caller's ticket. *(Opened 2026-09-10 at Q-0074's requirements
  gate as its GO-3, `draft`, p3.)* `finish()` sets `ticket.meta.iterations = context.counters` and
  advances `ticket.meta.stage` **before** its `if (!context.dry)` guard, and `types.ts:215` documents
  the counters as *"an alias, not a copy"*. **Nothing reaches disk** — `engine.ts:194` swaps in
  `readOnlyBacklog` under dry and `lifecycle.test.ts:228` pins that deliberately — so it is bounded
  to a caller reusing the ticket object in-process, and **there is exactly one such caller planned:
  M3's server**, which will answer `GET` from an object a `--dry` walk has just advanced. What it
  owes is a decision about ownership rather than a fix: does `runFlow` receive a ticket it may
  mutate, or a copy it may not? The cheap answer changes a landed preserved-defect pin, which is why
  it wants a ruling. Its tests must retain references to the original objects and assert them deeply
  unchanged — comparing serialised output after the run cannot see an alias. **Strictly before
  Q-0013**, and not to be folded into a git-probe ticket: the two share a file and nothing else.
