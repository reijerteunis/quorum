  ...
  **Each child body repeats Q-0010's ground rules verbatim**, because `input.backlog` resolves
  ...
  - Q-0105 Nothing checks whether `main` has been validated by CI. *(Opened 2026-09-05 from the
    ...
    **Two measurements are worth more than the criteria they support.** M-6 rules the print threshold:
    ...
    every locale.
    **Three gate obligations stand, all the human's, and the third is unique in this backlog.** GO-1,
    the decision entry before the first implement round, naming *"Containment is derived from git on
    each board invocation, never stored"* (2026-08-24) as the entry it extends — Q-0062's precedent is
    exact and cost three implement rounds. GO-2, `CLAUDE.md`'s term list, which no assertion in
    `packages/` reads. **GO-3 — push the merge and watch CI go green before this ticket closes**: every
    other ticket can be closed on a local verification, and a fix for *"nothing notices that `main` was
    never validated"* landing on an unpushed `main` is the defect closing its own ticket. **OQ-2 keeps
    the body's option 4 live** — record the gap as accepted and ship no code, under which the
    deliverable is GO-1's entry alone and all fourteen criteria are void.
