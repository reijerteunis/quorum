...
- **What a green tick does not claim, since Q-0079.** Both suites run on a machine whose git can
  ...
  `pnpm sweep:git-identity` is byte-identically what CI runs. It ran the spike suite too until
  Q-0107 AC-16, which removed that phase with the last workspace read of that tree; CI's own `spike`
  job still runs it until Q-0103, so what the sweep covers is smaller than it was and is stated here
  rather than left to be noticed. The **tripwire** is
