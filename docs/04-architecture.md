...
Since Q-0094 it dispatches six commands as well as its help — `lint` and `validate` from Q-0091, `runs` from Q-0092, `init` and `ticket` from Q-0093, which are the first two that write, and `run`, which is the one that executes. Each is one module named after the command the frame registers, which is what lets `frame.source.test.ts` derive the frame/command split from `COMMANDS` rather than from a list: **a frame module may name none of `core`'s domain helpers, and a command module only the ones its own command needs.** The presentation is the CLI's and the walk is `core`'s, so a lint record reaching a terminal, a browser and a WebSocket carries an escape byte in exactly one of the three.

`run` is where that division is widest, and it is the only command that needed no new `core` symbol at all: every helper it reaches was already on the public surface, and what the package owes is three things `core` deliberately does not do. It **renders** the trace — `runFlow` is a lazy, single-consumer `AsyncIterable<Event>` since Q-0050, so an exhaustive nine-arm switch turns one event into one line and a tenth member added to `shared`'s union is a compile error here. It **asks** the gate: the reader owns the scripted queue, the TTY test and the readline handle, and it throws `core`'s own `GateUnansweredError` rather than one of its own, because the run is classified `undecided` on that type and on nothing else. And it **installs the signal handler**, which `core` may not — the caller's `AbortSignal` is the whole cancellation mechanism, so `SIGINT` and `SIGTERM` are registered when a run starts and removed when it ends, in one file the package's own source guard names by identity. The reader takes its input stream, its output stream and its TTY predicate as parameters, which is what lets three of its five failure sites be tested without a build and without touching the real `process.stdin`.
