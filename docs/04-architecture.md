...
`{handle, flow, ticketId, runId, state, pendingGates, gates}`: the ticket narrows to its id, so no
ticket record crosses the wire and that field does not keep the name of the field it narrows.
...
through a question's `ticketDir` and through no row. **Host records are never pruned**, which a
