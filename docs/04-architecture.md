...
being true when `apps/web` gained a build task and is further from true since `packages/server`
gained one. That install carries neither the web app nor the daemon, and which of the two routes
gives it them is Q-0124's — a question that governs **two** packages rather than one.
...
`{handle, flow, ticketId, runId, state, pendingGates}`: the ticket narrows to its id and the gates to
a count, so no ticket record, event or gate question crosses the wire, and neither field keeps the
name of the field it narrows. `state` is the host's closed `refused | running | ended` rather than a
string. **Host records are never pruned**, which a listing is the first thing to make visible; that
...
that exists. **It emits and is not distributed**, which no package had been before: the emitting set
is five and the local distribution set stays three, so no tarball carries this package and a packed
`@quorum/cli` importing it would be broken — which is Q-0124's question rather than a silence, and
which becomes urgent at Q-0126. It keeps `private: true` and declares no `files` and no `bin`. See
*"A fifth package emits, and `resolved` is not a synonym for `distributed`"* (2026-09-12), which
extends *"A fourth package emits, and what it emits is served rather than shipped"* (2026-09-12).
Q-0125 adds no consumer at all: nothing in this workspace declares a dependency on it yet.
...
**`quorum open` starts the daemon against this project, serves the built web app and prints one URL, and it stops when you do.** It is the only command that does not return: `SIGINT` and `SIGTERM` close the daemon through `createDaemon`'s own `close()`, which releases every live run through the abandonment path before the socket stops answering, and it exits 130. The bind is `BIND_HOSTNAME` and no flag moves it; `--port <n>` overrides the one declared default, and a port something else holds **refuses naming that port** rather than drifting to another, because the dev proxy and any bookmarked URL both assume the one they were given. **What it does not do**: it runs no build — a directory holding no `index.html` refuses before anything binds, naming the directory and the file it wanted — it opens no browser, it discovers no already-running daemon, and it persists nothing. It does not make *"CLI and UI can both answer the same gate"* true either; it makes the UI reachable, and the gate screen is Q-0016's.

**It is also the first command that may be unavailable at run time, and that is a packaging fact rather than a defect.** `@quorum/cli` declares `@quorum/server` under **`optionalDependencies`** and reaches it through a **dynamic import inside the handler**; both halves are required and neither rescues the other. A required edge kills the packed install outright — `pnpm pack` rewrites `workspace:*` to `0.0.0`, and npm then reaches a registry for a package that is not published — while a static specifier would make `quorum help` die on an installation that skipped the optional one, because `main.ts` loads every command module whatever command was typed. On an installation that did skip it, `quorum open` reports **what failed to resolve here** and names the workspace path that carries the daemon; it may not report that the daemon is missing, broken or deliberately omitted, none of which an import that did not resolve can establish. The arrangement is **provisional against Q-0124**, which owns how an installation outside this workspace obtains the UI or the daemon. See *"An optional edge says the daemon may be absent, and never why"* (2026-09-14).
...
**The run route holds one live connection to the daemon.** Its frame parser validates every envelope and event, its connection state gives every outcome explicit text in the top bar, and its controller owns at most one socket, closes it on replacement or departure, and reconnects only through an explicit retry that preserves accepted evidence. The frame union has one browser-safe definition in `@quorum/shared`; `packages/server` imports and re-exports it so the server and browser share that contract without either end declaring a dependency on the other. That clause used to give as its reason that the daemon package had no browser-facing export surface at all, which Q-0125 ended by giving it one: the arrangement is unchanged and what holds it is that `@quorum/shared` is the package a browser may import, not that the daemon publishes nothing. All connection data remains in memory. **The app emits, and what it emits is served rather than shipped** (Q-0122): it declares `rm -rf dist && vite build`, so it is one of five emitters where the local distribution set stays three, and `packages/server` serves that output at `GET /*`. It keeps `private: true` and declares no `exports`, `files` or `bin`, so no tarball carries it and a packed install still has no web app — which is Q-0124's, not a silence. See *"A fourth package emits, and what it emits is served rather than shipped"* (2026-09-12), and **Emitted artifact** in the glossary, which that entry widened to cover a served bundle rather than coining a third kind. `eslint.config.js` gained `apps/**/*.tsx` in the same change as the shell: a flat-config `*.ts` tail does not match `.tsx`, so the three rules this workspace enforces reached no line of the app while `lint` reported green over it.
...
- **What a cache hit *gives back*, since Q-0097.** The three tasks above declare `"outputs": []`, so
  ...
  non-empty `outputs` in the workspace. **Five packages emit and three are packed**, which since
  2026-09-12 are two different sets: `@quorum/shared`, `@quorum/core`, `@quorum/cli` and — since
  ...
  fifth and declares `vite build`, whose output is **served** rather than resolved and which is in
  no tarball. **What emits and what ships are two facts, and after Q-0125 two packages are the
  difference rather than one**: `@quorum/web` is *served* and not distributed, `@quorum/server` is
  *resolved* and not distributed — the first artifact to be resolved without being packed, which is
  what separated the two axes for good. Each script removes its own emit directory before compiling,
