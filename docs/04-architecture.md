...
- **Until the cutover there are two required suites, and they have different reach.** The
  ...
  subset of the other. 53% of `spike/test/`, by line, spawns `spike/bin/harness.js`, and **that half
  transfers at Q-0010** rather than at Q-0054 — the CLI does not exist in `packages/` yet, so there
  is nothing for it to be aimed at. `packages/core/src/spike-parity.test.ts` is the file-by-file
