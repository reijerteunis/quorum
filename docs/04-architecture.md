...
- **What a cache hit *gives back*, since Q-0097.** The three tasks above declare `"outputs": []`, so
  ...
  *resolved* and not distributed — the first artifact to be resolved without being packed, which is
  ...
  `@quorum/server`'s is consumed by nothing yet: it exists so that Q-0126's `quorum open` resolves
  to a file rather than to `ERR_MODULE_NOT_FOUND`.
