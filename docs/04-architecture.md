...
rather than *"the three emitting packages"*, which is what this sentence said until 2026-09-12: four
packages emit and three are packed, so naming the packed set by the emitting set's name stopped being
true when `apps/web` gained a build task. That install carries no web app, and which of the two
routes gives it one is Q-0124's.
...
**The run route holds one live connection to the daemon.** Its frame parser validates every envelope and event, its connection state gives every outcome explicit text in the top bar, and its controller owns at most one socket, closes it on replacement or departure, and reconnects only through an explicit retry that preserves accepted evidence. The frame union has one browser-safe definition in `@quorum/shared`; `packages/server` imports and re-exports it so the server and browser share that contract without giving the server package a browser-facing export surface. All connection data remains in memory. **The app emits, and what it emits is served rather than shipped** (Q-0122): it declares `rm -rf dist && vite build`, so the emitting set is four where the local distribution set stays three, and `packages/server` serves that output at `GET /*`. It keeps `private: true` and declares no `exports`, `files` or `bin`, so no tarball carries it and a packed install still has no web app — which is Q-0124's, not a silence. See *"A fourth package emits, and what it emits is served rather than shipped"* (2026-09-12), and **Emitted artifact** in the glossary, which that entry widened to cover a served bundle rather than coining a third kind. `eslint.config.js` gained `apps/**/*.tsx` in the same change as the shell: a flat-config `*.ts` tail does not match `.tsx`, so the three rules this workspace enforces reached no line of the app while `lint` reported green over it.
...
- **What a cache hit *gives back*, since Q-0097.** The three tasks above declare `"outputs": []`, so
  ...
  non-empty `outputs` in the workspace. **Four packages emit and three are packed**, which since
  2026-09-12 are two different sets: `@quorum/shared`, `@quorum/core` and `@quorum/cli` each declare
  ...
  fourth and declares `vite build`, whose output is **served** rather than resolved and which is in
  no tarball. Each script removes its own emit directory before compiling, because turbo prunes an
  output directory on neither the miss path nor the hit path, so a file whose source has gone would
  otherwise survive both — measured, and for the three that is the whole mechanism, while Vite
  ...
  Q-0095's end-to-end suite, and — for the fourth — by a browser over the daemon's static route.
