...
await scenario('AC-5/AC-10/AC-11/AC-12/EDGE-4/EDGE-5', 'buildPrompt materialises the configured three-dot diff safely', () => {
  const f = fixture();
  ...
  assert.equal(Buffer.from(prompt).toString('utf8'), prompt, 'prompt ends on a valid UTF-8 boundary');
});
...
await scenario('AC-13/AC-14/AC-15/AC-16', 'three separate rejection rounds regress to the target consumes stage and persist exact counts', async () => {
  ...
  for (let n = 1; n <= 3; n++) {
    ...
    assert.equal(result.status, 'regressed'); assert.equal(result.stage, 'red'); assert.equal(f.ticket.meta.iterations.review, n);
  }
  assert.equal(f.messages.some((m) => /development.*red/.test(m)), true);
});

await scenario('AC-17/AC-18/EDGE-1/EDGE-10/EDGE-12', 'fourth rejection exhausts; retry persists max and grants one traversal only', async () => {
  const f = fixture(); const flow = loadFlow(path.join(f.harnessDir, 'flows', 'review.yaml'));
  ...
  assert.equal(result.status, 'regressed'); assert.equal(f.ticket.meta.iterations.review, 3);
  const exhausted = f.ticket.meta.history.filter((h) => h.status === 'exhausted');
...
await scenario('AC-29/EDGE-13', 'suite discovery and frozen contracts remain intact with no new dependency', () => {
  ...
  assert.equal(git(repo, 'diff', '--', 'contracts/Q-0006'), '');
});
