...
- `MOCK_RUN_HISTORY_PROFILES` may be a JSON object keyed by role whose values override
  ...
  malformed profiles fail explicitly. A selected profile sets the per-call `result.vendor` on
  success and `error.vendor` on a billed failure; the wrapper then copies that declaration into
  `usage.vendor` before applying its static-adapter fallback.
- Every mock-backed occurrence returns normal text, structured output, and aggregate usage through
  ...
  persisted by Q-0011.
- `MOCK_FAIL_WRITE` remains the billed-failure fixture. Its thrown error carries the selected
  profile's `error.vendor`; its usage carries the selected cache fields through the same wrapper
  as every other billed failure, and exposes the actual adapter-attempt count.
