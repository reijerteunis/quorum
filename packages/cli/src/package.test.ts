...
describe('AC-10(a) — this suite reads two repository files, and declares both', () => {
  ...
  const OUTSIDE: Record<string, string> = {
    ...
    'pnpm-workspace.yaml': 'build.test.ts, end-to-end.test.ts and failure-paths.test.ts, through the copier all three share in test/workspace.ts — Q-0097 AC-8, one of the four files that make the isolated copy a workspace turbo can plan',
   ...
   *
   * **Q-0103 removed four of the five Q-0107 added**, because it deleted the one file that read
   * them — `src/spike-dependencies.test.ts`, whose key set was the tree the cutover deletes. Those
   * four are `harness/architecture.md`, the per-package test and manifest globs, and the listing
   * input over the deleted tree; none has a reader in this package now, and the two package
   * manifests are named above and reach this task through the `^test` edge, as they did before that
   * register existed. The fifth — the per-package turbo configuration glob — stays with a different
   * reader named: `build.test.ts` reads every package's configuration to prove none of them declares
   * more than `inputs`, and no edge covers the four scaffold packages it also reaches. (The four are
   * described rather than spelled: a glob's separator closes a JSDoc block, which is how the first
   * draft of this paragraph turned the file into a parse error.)
   */
  const DECLARED = [
...
describe('Q-0096 AC-2 — the barrel exports the public API, so the trap closes rather than moves', () => {
  ...
  test('Q-0105 AC-12 — the surface grew by exactly one, and it is the name the board reaches', async () => {
    ...
    // Q-0127 moved them to 29 and 34, and all three of its names are `pathInside`'s case rather than
    // `openUrl`'s: no command reaches `listTicketFiles`, `readTicketFileBytes` or `isOneName`, and
    // the consumer is `packages/server`'s route for one ticket. What that costs is the same thing
    // Q-0122 wrote down — a `packages/cli` register grew for a `packages/server` need — and what it
    // buys is the same too: the frame is still forbidden to reimplement all three, which is this
    // register's own sentence and is true of them.
