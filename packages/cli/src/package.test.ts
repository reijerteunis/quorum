...
describe('AC-10(a) — this suite reads two repository files, and declares both', () => {
  ...
  const OUTSIDE: Record<string, string> = {
    ...
    'packages/core/src/adapters/mock.ts': 'end-to-end.test.ts — Q-0095 AC-9, the mock\'s nine switches are read out of the adapter that reads them, so the environment each spawned invocation is handed can be stripped of every one; a switch this list missed would let a shell force the verdict of the three convergent behaviours AC-5 asserts',
    'packages/core/src/backlog/backlog.ts': 'end-to-end.test.ts — Q-0095 AC-9, the same derivation reaches the owner default at :190, the preserved defect that takes an unpassed owner from the account',
