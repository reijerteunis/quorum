...
const MANIFEST: Record<string, Record<string, string>> = {
  '@quorum/shared#test': {
    ...
    'docs/USAGE.md': 'docs.test.ts — Q-0068 AC-6, the user guide\'s quotation of the BYOS refusal held against the literal packages/core/src/adapters/claude.ts throws, so neither side can move without the other',
    ...
    'packages/core/src/adapters/adapters.ts': 'project.test.ts — Q-0058 AC-2, withRetry\'s defaults are the oracle for the shipped example, so a change to one must move this task\'s hash; events.test.ts — Q-0107 AC-9, the retry event this union is derived from; and docs.test.ts — Q-0068 AC-14, ProbeResult\'s two nullable measures, which docs/03-adapter-contract.md is held against rather than being left to agree with itself',
    'packages/core/src/adapters/claude.ts': 'events.test.ts — Q-0107 AC-9, the spawn and stdout lines the adapter-event samples are taken from, re-aimed off spike/src/adapters/claude.js; and docs.test.ts — Q-0068 AC-6, the BYOS refusal docs/USAGE.md quotes',
    'packages/core/src/adapters/codex.ts': 'events.test.ts — Q-0107 AC-9, the same, for the second vendor; and docs.test.ts — Q-0068 AC-6, its own BYOS refusal, which shares one tail with claude\'s',
...
const NOT_READ: Record<string, string> = {
  ...
  'apps/web': 'the same register again, gained when Q-0122 made the emitting set four; the manifest behind that derivation is read through the `apps` walk WALKS already declares, and nothing opens the directory. This row said it was one of the TWO members that are NOT distributed until Q-0124 packed both, so the emitting set and the local distribution set are the same five again — see "The distribution set is five, and rejoins the emitting set" (2026-09-15). What survives that is the *shape*: this emit is **served** and the other four are **resolved**, which 093 defines by mechanism rather than by membership precisely so a sixth emitter can part the sets again. None of it changes why this is data rather than a read',
  'packages/server': 'the same register again, gained when Q-0125 made the emitting set five; the manifest behind that derivation is read through the `packages` walk WALKS already declares, and nothing opens the directory. It was the first artifact that was *resolved* and not distributed, which is what made the two axes independent; Q-0124 distributes it, so it is resolved AND distributed now and the independence is a fact about the axes rather than about this package — see "The distribution set is five, and rejoins the emitting set" (2026-09-15), which closes the question "A fifth package emits, and `resolved` is not a synonym for `distributed`" (2026-09-12) left open',
