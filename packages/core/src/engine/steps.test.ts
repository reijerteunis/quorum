...
describe('Q-0052 AC-4a — a role\'s default model never crosses vendors', () => {
  ...
  test('a role naming no adapter keeps its model, and a role naming no model passes none', () => {
    // The two rows the spike's three assertions leave out, and the branch each turns on: the guard
    // is `roleAdapter && roleAdapter !== adapterName`, so an absent adapter must NOT suppress the
    // model, and an absent model must not become a string.
