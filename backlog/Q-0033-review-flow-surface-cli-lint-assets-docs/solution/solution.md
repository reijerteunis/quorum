...
Only `--gate-answer` becomes accumulating and repeatable; all other flags keep their current last-wins behavior. Gate answers are consumed once each in encounter order. Values supplied through `--gate-answer` must normalize to exactly `advance`, `retry`, or `abort`, with no prefixes or abbreviations, and are further restricted to the options offered by the encountered gate.

After explicit answers are exhausted, prompting is permitted only on a TTY. The existing forgiving interactive abbreviations may remain: an invalid interactive response re-prompts the human and never invents a decision. Non-TTY input and missing, empty, invalid, or unavailable explicit answers fail immediately with a diagnostic naming the gate. No path defaults to `advance`. `--auto` remains a run policy and does not manufacture an answer for an exhaustion gate.
...
The tasks remain serialized, matching the inherited pre-split ordering: `Q0033-assets-docs` follows `Q0033-cli-lint-config`. Their production files are disjoint, but both use the broad `backend` role whose write allowance covers `spike/`, `harness/`, and `docs/`; serialization ensures each agent’s assigned files are not simultaneously inside another active agent’s writable area with only prose ownership separating them. It also preserves the previously settled CLI-before-assets boundary, so the shipped flow is evaluated against the completed shared validator.
...
- **Running the two implementation tasks in parallel:** rejected despite disjoint declared production ownership because both use the broad backend write allowance. Serial execution makes the prose ownership boundary enforceable operationally and retains the inherited CLI-before-assets order.
...
- **Empty input, prefixes, or `--auto` as explicit gate decisions:** rejected because scripted gate choices may never be silently invented. Forgiving TTY abbreviations remain an interactive convenience and re-prompt rather than defaulting.
...
- the gate-dependent smoke assertion currently at `spike/test/smoke.js:82-85`, providing an explicit answer where needed while retaining its `loop exhausted` expectation;
...
- **R2.1:** scoped exact full-word matching to explicit `--gate-answer` values and retained forgiving, re-prompting TTY input.
- **R2.2:** named the gate-dependent `spike/test/smoke.js:82-85` assertion in qa-red guidance.
- **R2.3:** replaced the serialization rationale with the actual broad backend write-allowance risk.
- **R2.4:** restored the established top-level `tasks:` key in the Tasks YAML.
