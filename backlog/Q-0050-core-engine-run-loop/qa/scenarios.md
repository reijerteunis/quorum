...
| AC-3a | q0050-lifecycle, q0050-engine-compose | `engine.test.ts` — all five statuses driven through `runFlow`, and since round 6 the `failed` one asserts the terminal event **before** the rejection — `collect` returns its array only on normal completion, so the one status where AC-3's "additionally throws" has a subject had none: completed (banner test), regressed (AC-8b), aborted (AC-3a), failed (AC-8c), interrupted (AC-5c/5d and the abort-reason test) | `terminalInfo` |
...
| AC-4a–4h | q0050-routing, q0050-shared-events | `lifecycle-routing.test.ts` (4a–4g at the `askGate` unit level. 4c constructs a genuine replay — one context, first gate resolved — beside a never-issued id, and asserts each error names its own id and not the other's; 4d and 4e were deleted by accident in round 4's 4c fix and restored in round 5, 4e having had no other coverage anywhere in `packages/core`; 4g reads `log.gateAnswer` whole), `engine.test.ts` (4a's passive consumer and 4f's `--auto` clause, at the composed level), `q0050.source.test.ts` (4h). **4b's "the run resumes and completes" half is asserted at the unit level only**, and **4d is superseded by `solution/errata.md` E-19** — the code refuses an unparseable answer where the criterion says it aborts | `gateAutoAdvanced`, `gateDryRun`, `log.gateAnswer` |
...
| AC-6 `vars.iter` | q0050-engine-compose | `engine.test.ts` — AC-6: increments on the intra-flow branch, not on the cross-flow one. Added 2026-08-29 after round 6 measured `iter` at three sites in the engine and **none** in any test file, so both halves were free | — |
...
| AC-13d | all tasks | `q0050.source.test.ts` — AC-13d: a register of **all ten** `Why: preserved …` markers by file, kind and authority — seven `defect/`, which is AC-13d's own enumeration, plus a `design/` and two `behaviour|behavior/` that a defect-only scan could not see (E-20). The "reproduces no sentence" half is now the real substring scan against `docs/DECISIONS.md` and `ticket.md`, `backlog/*/ticket.md` already an input and `docs/DECISIONS.md` declared alongside the scan, demonstrated red before being trusted — it replaced a length proxy any short copied sentence passed, asserted as an identity map rather than a count | — |
