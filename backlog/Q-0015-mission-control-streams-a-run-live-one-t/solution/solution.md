...
*Final solution, run 2, 2026-09-16. This document supersedes drafts 1–4; its Contracts and Tasks sections are the only versions to use.*
...
This preserves AC-6’s prohibition on deriving machine values from human prose without weakening the existing accessibility assertion. QA receives the ruling through `solution/errata.md`; implementers receive the same twelve-needle rule through `contracts/Q-0015/mission-control.contract.md` after the contracts branch is merged into the integration branch.
...
- QA rewrites the existing `/runs` route assertions so the row is assigned to `Q-0015`, its obsolete “No ticket builds this screen yet” expectation is removed, and its non-empty replacement `waitingFor` sentence is asserted directly rather than constrained by the stale `/listing/` wording.
- The `app.tsx` route-binding guard is extended or derived from every register row with `screenExists: true`, so it covers `RUNS_PATH` and `RUN_ROUTE` as well as the existing three routes; removing either new constant from `app.tsx` must make the guard red.
