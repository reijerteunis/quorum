...
5. **The `steps`-less flow is not fixed here**, though the body calls it *"the same class"*. It is:
...
**GO-2.** Rule OQ-2 — keep the `steps`-less rule out and open its successor, or fold it in — **at the
gate**, and record the answer. An obligation left in this document's prose is the drift this
repository has found seven directions of.

**GO-3.** Open the successor for non-goal 7 — a flow that fails lint vanishes from `quorum board`'s
runnable-flow index with no explanation — or record at the gate that it is accepted.
