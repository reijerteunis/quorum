...
Before `harness run qa-red Q-0011`, the maintainer must replace the remaining reference to “an events schema” in `backlog/Q-0011-run-history-on-disk/ticket.md` with “a run manifest schema”. The engine includes the ticket body verbatim in downstream prompts, so leaving that sentence unchanged would contradict the approved scope. No implementation task owns `ticket.md`.
...
This annotation-driven semantic pass extends the repository’s contract-validation model and must be recorded in `docs/DECISIONS.md`; the backend task owns that documentation while the tooling task implements the capability.
...
tasks:
  - id: q0011-engine-writer
    ...
    description: >
      ...
      referenced frozen contracts. Document the expanded adapter usage/result/error boundary,
      the .quorum/runs/ layout, the run-history glossary term, and the annotation-driven
      semantic-validation decision. Do not edit contracts/Q-0011/**, spike/bin/**,
    ...
    contracts:
      ...
      - backlog/Q-0011-run-history-on-disk/solution/errata.md
    depends_on: []
    owns:
      - spike/src/**
...
- **B-1:** The writer takes vendor provenance from `result.vendor` or `error.vendor`, with static `adapter.vendor` only as fallback. The mock profile explicitly sets both per-call success and failure declarations.
- **B-2:** The approved solution contains no event-file or event-schema contract. The maintainer must correct the one stale “events schema” phrase in `ticket.md` before qa-red because implementation tasks are intentionally forbidden to edit that file.
...
- **M-5:** `docs/03-adapter-contract.md` is owned by the backend task, whose description now explicitly requires documenting the expanded usage/result/error boundary.
...
- **Delivery of errata:** Both implementation tasks reference `solution/errata.md`, ensuring E-1 through E-3 are included in their task prompts.
- **Documentation ownership:** The backend task explicitly covers the run-history glossary term, `.quorum/runs/` architecture, adapter boundary, and semantic-validation decision record.
